...
import { isAnswerCorrect, maxPoints, scoreOn20 } from "@/lib/grading";
...
export async function POST(request: NextRequest) {
  ...
  const graded = problem.questions.map((question) => {
    ...
    if (!response || response.skipped) {
      return { questionId: question.id, status: "SKIPPED" as const, hintsUsed: 0, pointsEarned: 0 };
    }

    const hintsUsed = Math.min(response.hintsUsed, question.hints.length);
    const correct = isAnswerCorrect(question, response.rawAnswer);
    if (!correct) {
      return { questionId: question.id, status: "INCORRECT" as const, hintsUsed, pointsEarned: 0 };
    }

    const malus = question.hints.slice(0, hintsUsed).reduce((sum, hint) => sum + hint.malus, 0);
    return {
      questionId: question.id,
      status: "CORRECT" as const,
      hintsUsed,
      pointsEarned: Math.max(0, question.points - malus),
    };
  });
