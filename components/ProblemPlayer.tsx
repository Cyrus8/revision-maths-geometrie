...
export function ProblemPlayer({
  ...
  const isLast = index === problem.questions.length - 1;
  ...
  function validate() {
    ...
    if (isAnswerCorrect(question, draft)) {
      ...
      recordOutcome(question.id, { status: "correct", hintsUsed: hintsShown });
      setFeedback("correct");
    } else {
      setFeedback("wrong");
    }
  ...
  function skip() {
    recordOutcome(question.id, { status: "skipped", hintsUsed: hintsShown });
    setFeedback("idle");
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <Stepper questions={problem.questions} outcomes={outcomes} currentIndex={index} />

      <Card className="p-5 sm:p-6">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <Badge tone="accent">Question {index + 1} / {problem.questions.length}</Badge>
        ...
        {feedback === "wrong" && !resolved && (
          <p className="mt-3 text-sm text-danger">
            Pas encore correct. Vérifie ta méthode et réessaie, ou demande un indice.
          </p>
        )}
    ...
    <div className="flex flex-wrap gap-2">
      {questions.map((q, i) => {
        ...
        return (
          <span
            key={q.id}
            className={`flex h-9 w-9 items-center justify-center rounded-full border text-sm font-medium ${classes}`}
          >
            {label}
          </span>
        );
